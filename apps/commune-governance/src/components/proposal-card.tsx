...
import { Vote, VoteLabel } from "./vote-label";
import {
  bigintDivision,
  formatToken,
  smallAddress,
...
function handleFavorablePercent(favorablePercent: number) {
...
function renderFavorablePercent(stakeInfo: ProposalStakeInfo) {
...
function renderQuorumPercent(stakeInfo: ProposalStakeInfo) {
  const { stakeVoted, stakeTotal } = stakeInfo;
  const quorum_percent = bigintDivision(stakeVoted, stakeTotal) * 100;
  return (
