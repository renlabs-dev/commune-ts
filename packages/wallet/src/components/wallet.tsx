      ...
      {!openSelectWalletModal && (
        ...
        >
          ...
          <div className={cn("flex p-4")}>
            <div className={cn("flex border border-gray-500 w-full")}>
              {walletActions.map((action) => {
                return (
                  <button
                    className={cn(
                      `flex w-1/3 flex-col items-center border-gray-500 px-3.5 py-3 text-gray-400 ${activeMenu == action.name.toLocaleLowerCase() ? action.bgColor : ""}`,
                    )}
                    key={action.name}
                    onClick={() => {
